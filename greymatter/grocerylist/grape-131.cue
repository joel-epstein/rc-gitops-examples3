...
Grape_131: {
	...
	config: [
		...
		// Grape_131 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Grape_131 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/grape-131/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/grape-131$"
					to:            route_match.path
		...
		// greymatter catalog service definition for grape-131
		#catalog_entry & {
			name:                      Name
		...
		// Shared grape-131 proxy object. All configs
