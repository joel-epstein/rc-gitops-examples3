...
Fig_150: {
	...
	config: [
		...
		// Fig_150 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_150 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-150/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-150$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-150
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-150 proxy object. All configs
