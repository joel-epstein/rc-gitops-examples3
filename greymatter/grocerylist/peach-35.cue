...
Peach_35: {
	...
	config: [
		...
		// Peach_35 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Peach_35 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/peach-35/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/peach-35$"
					to:            route_match.path
		...
		// greymatter catalog service definition for peach-35
		#catalog_entry & {
			name:                      Name
		...
		// Shared peach-35 proxy object. All configs
