...
Grape_148: {
	...
	config: [
		...
		// Grape_148 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Grape_148 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/grape-148/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/grape-148$"
					to:            route_match.path
		...
		// greymatter catalog service definition for grape-148
		#catalog_entry & {
			name:                      Name
		...
		// Shared grape-148 proxy object. All configs
