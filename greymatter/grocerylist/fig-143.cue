...
Fig_143: {
	...
	config: [
		...
		// Fig_143 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_143 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-143/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-143$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-143
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-143 proxy object. All configs
