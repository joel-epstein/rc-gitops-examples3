...
Fig_139: {
	...
	config: [
		...
		// Fig_139 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_139 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-139/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-139$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-139
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-139 proxy object. All configs
