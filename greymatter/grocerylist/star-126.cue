...
Star_126: {
	...
	config: [
		...
		// Star_126 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_126 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-126/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-126$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-126
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-126 proxy object. All configs
