...
Fig_104: {
	...
	config: [
		...
		// Fig_104 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_104 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-104/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-104$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-104
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-104 proxy object. All configs
