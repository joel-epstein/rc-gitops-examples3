...
Star_119: {
	...
	config: [
		...
		// Star_119 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_119 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-119/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-119$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-119
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-119 proxy object. All configs
