...
Peach_67: {
	...
	config: [
		...
		// Peach_67 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Peach_67 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/peach-67/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/peach-67$"
					to:            route_match.path
		...
		// greymatter catalog service definition for peach-67
		#catalog_entry & {
			name:                      Name
		...
		// Shared peach-67 proxy object. All configs
