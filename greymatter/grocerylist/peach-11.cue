...
Peach_11: {
	...
	config: [
		...
		// Peach_11 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Peach_11 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/peach-11/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/peach-11$"
					to:            route_match.path
		...
		// greymatter catalog service definition for peach-11
		#catalog_entry & {
			name:                      Name
		...
		// Shared peach-11 proxy object. All configs
