...
Star_136: {
	...
	config: [
		...
		// Star_136 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_136 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-136/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-136$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-136
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-136 proxy object. All configs
