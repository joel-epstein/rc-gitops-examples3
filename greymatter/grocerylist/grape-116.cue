...
Grape_116: {
	...
	config: [
		...
		// Grape_116 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Grape_116 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/grape-116/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/grape-116$"
					to:            route_match.path
		...
		// greymatter catalog service definition for grape-116
		#catalog_entry & {
			name:                      Name
		...
		// Shared grape-116 proxy object. All configs
