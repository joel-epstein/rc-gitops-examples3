...
Grape_110: {
	...
	config: [
		...
		// Grape_110 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Grape_110 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/grape-110/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/grape-110$"
					to:            route_match.path
		...
		// greymatter catalog service definition for grape-110
		#catalog_entry & {
			name:                      Name
		...
		// Shared grape-110 proxy object. All configs
