...
Grape_129: {
	...
	config: [
		...
		// Grape_129 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Grape_129 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/grape-129/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/grape-129$"
					to:            route_match.path
		...
		// greymatter catalog service definition for grape-129
		#catalog_entry & {
			name:                      Name
		...
		// Shared grape-129 proxy object. All configs
