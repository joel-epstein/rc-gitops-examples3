...
Fig_109: {
	...
	config: [
		...
		// Fig_109 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_109 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-109/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-109$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-109
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-109 proxy object. All configs
