...
Star_112: {
	...
	config: [
		...
		// Star_112 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_112 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-112/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-112$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-112
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-112 proxy object. All configs
