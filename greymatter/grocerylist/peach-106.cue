...
Peach_106: {
	...
	config: [
		...
		// Peach_106 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Peach_106 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/peach-106/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/peach-106$"
					to:            route_match.path
		...
		// greymatter catalog service definition for peach-106
		#catalog_entry & {
			name:                      Name
		...
		// Shared peach-106 proxy object. All configs
