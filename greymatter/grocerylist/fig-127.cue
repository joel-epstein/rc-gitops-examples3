...
Fig_127: {
	...
	config: [
		...
		// Fig_127 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_127 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-127/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-127$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-127
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-127 proxy object. All configs
