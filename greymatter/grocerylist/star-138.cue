...
Star_138: {
	...
	config: [
		...
		// Star_138 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_138 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-138/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-138$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-138
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-138 proxy object. All configs
