...
Peach_63: {
	...
	config: [
		...
		// Peach_63 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Peach_63 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/peach-63/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/peach-63$"
					to:            route_match.path
		...
		// greymatter catalog service definition for peach-63
		#catalog_entry & {
			name:                      Name
		...
		// Shared peach-63 proxy object. All configs
