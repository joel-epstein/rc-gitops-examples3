...
Star_127: {
	...
	config: [
		...
		// Star_127 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_127 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-127/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-127$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-127
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-127 proxy object. All configs
