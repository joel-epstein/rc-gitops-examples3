...
Peach_73: {
	...
	config: [
		...
		// Peach_73 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Peach_73 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/peach-73/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/peach-73$"
					to:            route_match.path
		...
		// greymatter catalog service definition for peach-73
		#catalog_entry & {
			name:                      Name
		...
		// Shared peach-73 proxy object. All configs
