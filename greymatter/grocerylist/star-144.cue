...
Star_144: {
	...
	config: [
		...
		// Star_144 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_144 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-144/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-144$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-144
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-144 proxy object. All configs
