...
Fig_140: {
	...
	config: [
		...
		// Fig_140 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_140 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-140/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-140$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-140
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-140 proxy object. All configs
