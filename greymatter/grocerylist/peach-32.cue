...
Peach_32: {
	...
	config: [
		...
		// Peach_32 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Peach_32 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/peach-32/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/peach-32$"
					to:            route_match.path
		...
		// greymatter catalog service definition for peach-32
		#catalog_entry & {
			name:                      Name
		...
		// Shared peach-32 proxy object. All configs
