...
Grape_103: {
	...
	config: [
		...
		// Grape_103 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Grape_103 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/grape-103/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/grape-103$"
					to:            route_match.path
		...
		// greymatter catalog service definition for grape-103
		#catalog_entry & {
			name:                      Name
		...
		// Shared grape-103 proxy object. All configs
