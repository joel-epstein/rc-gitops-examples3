...
Peach_110: {
	...
	config: [
		...
		// Peach_110 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Peach_110 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/peach-110/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/peach-110$"
					to:            route_match.path
		...
		// greymatter catalog service definition for peach-110
		#catalog_entry & {
			name:                      Name
		...
		// Shared peach-110 proxy object. All configs
