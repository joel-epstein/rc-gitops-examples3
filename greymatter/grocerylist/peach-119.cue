...
Peach_119: {
	...
	config: [
		...
		// Peach_119 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Peach_119 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/peach-119/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/peach-119$"
					to:            route_match.path
		...
		// greymatter catalog service definition for peach-119
		#catalog_entry & {
			name:                      Name
		...
		// Shared peach-119 proxy object. All configs
