...
Peach_84: {
	...
	config: [
		...
		// Peach_84 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Peach_84 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/peach-84/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/peach-84$"
					to:            route_match.path
		...
		// greymatter catalog service definition for peach-84
		#catalog_entry & {
			name:                      Name
		...
		// Shared peach-84 proxy object. All configs
