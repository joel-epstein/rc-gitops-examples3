...
Peach_3: {
	...
	config: [
		...
		// Peach_3 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Peach_3 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/peach-3/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/peach-3$"
					to:            route_match.path
		...
		// greymatter catalog service definition for peach-3
		#catalog_entry & {
			name:                      Name
		...
		// Shared peach-3 proxy object. All configs
