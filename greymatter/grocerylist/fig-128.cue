...
Fig_128: {
	...
	config: [
		...
		// Fig_128 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_128 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-128/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-128$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-128
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-128 proxy object. All configs
