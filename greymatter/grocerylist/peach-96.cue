...
Peach_96: {
	...
	config: [
		...
		// Peach_96 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Peach_96 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/peach-96/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/peach-96$"
					to:            route_match.path
		...
		// greymatter catalog service definition for peach-96
		#catalog_entry & {
			name:                      Name
		...
		// Shared peach-96 proxy object. All configs
