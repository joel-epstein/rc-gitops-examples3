...
Fig_117: {
	...
	config: [
		...
		// Fig_117 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_117 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-117/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-117$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-117
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-117 proxy object. All configs
