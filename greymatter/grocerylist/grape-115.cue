...
Grape_115: {
	...
	config: [
		...
		// Grape_115 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Grape_115 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/grape-115/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/grape-115$"
					to:            route_match.path
		...
		// greymatter catalog service definition for grape-115
		#catalog_entry & {
			name:                      Name
		...
		// Shared grape-115 proxy object. All configs
