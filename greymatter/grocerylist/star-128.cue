...
Star_128: {
	...
	config: [
		...
		// Star_128 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_128 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-128/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-128$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-128
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-128 proxy object. All configs
