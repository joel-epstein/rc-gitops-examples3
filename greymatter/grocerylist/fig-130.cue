...
Fig_130: {
	...
	config: [
		...
		// Fig_130 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_130 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-130/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-130$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-130
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-130 proxy object. All configs
