...
Star_122: {
	...
	config: [
		...
		// Star_122 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_122 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-122/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-122$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-122
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-122 proxy object. All configs
