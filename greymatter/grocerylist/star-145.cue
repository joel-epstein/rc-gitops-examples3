...
Star_145: {
	...
	config: [
		...
		// Star_145 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_145 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-145/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-145$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-145
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-145 proxy object. All configs
