...
Star_107: {
	...
	config: [
		...
		// Star_107 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_107 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-107/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-107$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-107
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-107 proxy object. All configs
