...
Grape_136: {
	...
	config: [
		...
		// Grape_136 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Grape_136 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/grape-136/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/grape-136$"
					to:            route_match.path
		...
		// greymatter catalog service definition for grape-136
		#catalog_entry & {
			name:                      Name
		...
		// Shared grape-136 proxy object. All configs
