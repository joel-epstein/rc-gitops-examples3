...
Peach_81: {
	...
	config: [
		...
		// Peach_81 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Peach_81 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/peach-81/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/peach-81$"
					to:            route_match.path
		...
		// greymatter catalog service definition for peach-81
		#catalog_entry & {
			name:                      Name
		...
		// Shared peach-81 proxy object. All configs
