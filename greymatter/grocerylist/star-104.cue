...
Star_104: {
	...
	config: [
		...
		// Star_104 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_104 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-104/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-104$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-104
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-104 proxy object. All configs
