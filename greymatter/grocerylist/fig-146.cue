...
Fig_146: {
	...
	config: [
		...
		// Fig_146 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_146 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-146/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-146$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-146
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-146 proxy object. All configs
