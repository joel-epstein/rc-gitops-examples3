...
Fig_115: {
	...
	config: [
		...
		// Fig_115 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_115 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-115/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-115$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-115
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-115 proxy object. All configs
