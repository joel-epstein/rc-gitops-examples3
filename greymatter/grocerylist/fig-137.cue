...
Fig_137: {
	...
	config: [
		...
		// Fig_137 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_137 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-137/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-137$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-137
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-137 proxy object. All configs
