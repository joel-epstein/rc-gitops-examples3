...
Grape_150: {
	...
	config: [
		...
		// Grape_150 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Grape_150 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/grape-150/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/grape-150$"
					to:            route_match.path
		...
		// greymatter catalog service definition for grape-150
		#catalog_entry & {
			name:                      Name
		...
		// Shared grape-150 proxy object. All configs
