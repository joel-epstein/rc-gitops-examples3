...
Peach_104: {
	...
	config: [
		...
		// Peach_104 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Peach_104 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/peach-104/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/peach-104$"
					to:            route_match.path
		...
		// greymatter catalog service definition for peach-104
		#catalog_entry & {
			name:                      Name
		...
		// Shared peach-104 proxy object. All configs
