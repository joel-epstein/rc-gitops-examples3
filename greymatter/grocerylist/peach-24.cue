...
Peach_24: {
	...
	config: [
		...
		// Peach_24 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Peach_24 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/peach-24/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/peach-24$"
					to:            route_match.path
		...
		// greymatter catalog service definition for peach-24
		#catalog_entry & {
			name:                      Name
		...
		// Shared peach-24 proxy object. All configs
