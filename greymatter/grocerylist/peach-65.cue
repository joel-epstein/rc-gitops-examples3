...
Peach_65: {
	...
	config: [
		...
		// Peach_65 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Peach_65 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/peach-65/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/peach-65$"
					to:            route_match.path
		...
		// greymatter catalog service definition for peach-65
		#catalog_entry & {
			name:                      Name
		...
		// Shared peach-65 proxy object. All configs
