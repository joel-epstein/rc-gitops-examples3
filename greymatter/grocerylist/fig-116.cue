...
Fig_116: {
	...
	config: [
		...
		// Fig_116 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_116 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-116/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-116$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-116
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-116 proxy object. All configs
