...
Fig_112: {
	...
	config: [
		...
		// Fig_112 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_112 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-112/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-112$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-112
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-112 proxy object. All configs
