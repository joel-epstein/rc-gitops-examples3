...
Grape_143: {
	...
	config: [
		...
		// Grape_143 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Grape_143 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/grape-143/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/grape-143$"
					to:            route_match.path
		...
		// greymatter catalog service definition for grape-143
		#catalog_entry & {
			name:                      Name
		...
		// Shared grape-143 proxy object. All configs
