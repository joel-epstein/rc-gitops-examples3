...
Peach_19: {
	...
	config: [
		...
		// Peach_19 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Peach_19 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/peach-19/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/peach-19$"
					to:            route_match.path
		...
		// greymatter catalog service definition for peach-19
		#catalog_entry & {
			name:                      Name
		...
		// Shared peach-19 proxy object. All configs
