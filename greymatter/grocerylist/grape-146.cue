...
Grape_146: {
	...
	config: [
		...
		// Grape_146 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Grape_146 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/grape-146/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/grape-146$"
					to:            route_match.path
		...
		// greymatter catalog service definition for grape-146
		#catalog_entry & {
			name:                      Name
		...
		// Shared grape-146 proxy object. All configs
