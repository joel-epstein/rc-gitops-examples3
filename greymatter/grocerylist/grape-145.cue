...
Grape_145: {
	...
	config: [
		...
		// Grape_145 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Grape_145 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/grape-145/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/grape-145$"
					to:            route_match.path
		...
		// greymatter catalog service definition for grape-145
		#catalog_entry & {
			name:                      Name
		...
		// Shared grape-145 proxy object. All configs
