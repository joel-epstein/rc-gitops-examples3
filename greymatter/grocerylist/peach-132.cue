...
Peach_132: {
	...
	config: [
		...
		// Peach_132 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Peach_132 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/peach-132/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/peach-132$"
					to:            route_match.path
		...
		// greymatter catalog service definition for peach-132
		#catalog_entry & {
			name:                      Name
		...
		// Shared peach-132 proxy object. All configs
