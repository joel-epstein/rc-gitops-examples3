...
Fig_101: {
	...
	config: [
		...
		// Fig_101 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_101 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-101/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-101$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-101
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-101 proxy object. All configs
