...
Star_111: {
	...
	config: [
		...
		// Star_111 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_111 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-111/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-111$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-111
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-111 proxy object. All configs
