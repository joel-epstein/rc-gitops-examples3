...
Star_130: {
	...
	config: [
		...
		// Star_130 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_130 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-130/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-130$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-130
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-130 proxy object. All configs
