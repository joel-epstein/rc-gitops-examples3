...
Grape_107: {
	...
	config: [
		...
		// Grape_107 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Grape_107 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/grape-107/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/grape-107$"
					to:            route_match.path
		...
		// greymatter catalog service definition for grape-107
		#catalog_entry & {
			name:                      Name
		...
		// Shared grape-107 proxy object. All configs
