...
Peach_112: {
	...
	config: [
		...
		// Peach_112 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Peach_112 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/peach-112/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/peach-112$"
					to:            route_match.path
		...
		// greymatter catalog service definition for peach-112
		#catalog_entry & {
			name:                      Name
		...
		// Shared peach-112 proxy object. All configs
