...
Fig_111: {
	...
	config: [
		...
		// Fig_111 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_111 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-111/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-111$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-111
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-111 proxy object. All configs
