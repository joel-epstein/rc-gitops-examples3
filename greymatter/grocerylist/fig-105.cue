...
Fig_105: {
	...
	config: [
		...
		// Fig_105 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_105 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-105/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-105$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-105
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-105 proxy object. All configs
