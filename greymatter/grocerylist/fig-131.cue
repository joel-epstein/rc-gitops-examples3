...
Fig_131: {
	...
	config: [
		...
		// Fig_131 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_131 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-131/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-131$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-131
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-131 proxy object. All configs
