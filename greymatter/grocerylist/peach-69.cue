...
Peach_69: {
	...
	config: [
		...
		// Peach_69 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Peach_69 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/peach-69/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/peach-69$"
					to:            route_match.path
		...
		// greymatter catalog service definition for peach-69
		#catalog_entry & {
			name:                      Name
		...
		// Shared peach-69 proxy object. All configs
