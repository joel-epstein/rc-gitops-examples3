...
Peach_59: {
	...
	config: [
		...
		// Peach_59 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Peach_59 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/peach-59/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/peach-59$"
					to:            route_match.path
		...
		// greymatter catalog service definition for peach-59
		#catalog_entry & {
			name:                      Name
		...
		// Shared peach-59 proxy object. All configs
