...
Grape_137: {
	...
	config: [
		...
		// Grape_137 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Grape_137 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/grape-137/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/grape-137$"
					to:            route_match.path
		...
		// greymatter catalog service definition for grape-137
		#catalog_entry & {
			name:                      Name
		...
		// Shared grape-137 proxy object. All configs
