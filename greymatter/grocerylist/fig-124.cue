...
Fig_124: {
	...
	config: [
		...
		// Fig_124 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_124 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-124/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-124$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-124
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-124 proxy object. All configs
