...
Peach_9: {
	...
	config: [
		...
		// Peach_9 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Peach_9 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/peach-9/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/peach-9$"
					to:            route_match.path
		...
		// greymatter catalog service definition for peach-9
		#catalog_entry & {
			name:                      Name
		...
		// Shared peach-9 proxy object. All configs
