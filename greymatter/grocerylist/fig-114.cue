...
Fig_114: {
	...
	config: [
		...
		// Fig_114 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_114 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-114/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-114$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-114
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-114 proxy object. All configs
