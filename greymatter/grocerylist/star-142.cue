...
Star_142: {
	...
	config: [
		...
		// Star_142 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_142 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-142/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-142$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-142
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-142 proxy object. All configs
