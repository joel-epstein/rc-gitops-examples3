...
Peach_100: {
	...
	config: [
		...
		// Peach_100 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Peach_100 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/peach-100/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/peach-100$"
					to:            route_match.path
		...
		// greymatter catalog service definition for peach-100
		#catalog_entry & {
			name:                      Name
		...
		// Shared peach-100 proxy object. All configs
