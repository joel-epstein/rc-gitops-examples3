...
Star_123: {
	...
	config: [
		...
		// Star_123 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_123 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-123/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-123$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-123
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-123 proxy object. All configs
