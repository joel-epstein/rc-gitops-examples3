...
Star_120: {
	...
	config: [
		...
		// Star_120 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_120 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-120/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-120$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-120
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-120 proxy object. All configs
