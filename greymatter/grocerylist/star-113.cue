...
Star_113: {
	...
	config: [
		...
		// Star_113 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_113 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-113/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-113$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-113
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-113 proxy object. All configs
