...
Star_149: {
	...
	config: [
		...
		// Star_149 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_149 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-149/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-149$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-149
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-149 proxy object. All configs
