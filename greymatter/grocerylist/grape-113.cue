...
Grape_113: {
	...
	config: [
		...
		// Grape_113 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Grape_113 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/grape-113/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/grape-113$"
					to:            route_match.path
		...
		// greymatter catalog service definition for grape-113
		#catalog_entry & {
			name:                      Name
		...
		// Shared grape-113 proxy object. All configs
