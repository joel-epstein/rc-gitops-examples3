...
Star_118: {
	...
	config: [
		...
		// Star_118 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_118 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-118/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-118$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-118
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-118 proxy object. All configs
