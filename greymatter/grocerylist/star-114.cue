...
Star_114: {
	...
	config: [
		...
		// Star_114 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_114 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-114/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-114$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-114
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-114 proxy object. All configs
