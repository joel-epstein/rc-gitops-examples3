...
Peach_115: {
	...
	config: [
		...
		// Peach_115 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Peach_115 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/peach-115/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/peach-115$"
					to:            route_match.path
		...
		// greymatter catalog service definition for peach-115
		#catalog_entry & {
			name:                      Name
		...
		// Shared peach-115 proxy object. All configs
