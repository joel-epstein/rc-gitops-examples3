...
Fig_102: {
	...
	config: [
		...
		// Fig_102 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_102 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-102/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-102$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-102
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-102 proxy object. All configs
