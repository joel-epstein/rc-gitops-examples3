...
Fig_110: {
	...
	config: [
		...
		// Fig_110 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_110 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-110/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-110$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-110
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-110 proxy object. All configs
