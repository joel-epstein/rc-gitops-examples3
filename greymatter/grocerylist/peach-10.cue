...
Peach_10: {
	...
	config: [
		...
		// Peach_10 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Peach_10 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/peach-10/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/peach-10$"
					to:            route_match.path
		...
		// greymatter catalog service definition for peach-10
		#catalog_entry & {
			name:                      Name
		...
		// Shared peach-10 proxy object. All configs
