...
Peach_139: {
	...
	config: [
		...
		// Peach_139 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Peach_139 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/peach-139/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/peach-139$"
					to:            route_match.path
		...
		// greymatter catalog service definition for peach-139
		#catalog_entry & {
			name:                      Name
		...
		// Shared peach-139 proxy object. All configs
