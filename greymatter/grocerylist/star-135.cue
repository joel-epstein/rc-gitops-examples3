...
Star_135: {
	...
	config: [
		...
		// Star_135 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_135 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-135/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-135$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-135
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-135 proxy object. All configs
