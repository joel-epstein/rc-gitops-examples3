...
Star_106: {
	...
	config: [
		...
		// Star_106 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_106 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-106/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-106$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-106
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-106 proxy object. All configs
