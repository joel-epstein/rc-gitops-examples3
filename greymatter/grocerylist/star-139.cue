...
Star_139: {
	...
	config: [
		...
		// Star_139 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_139 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-139/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-139$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-139
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-139 proxy object. All configs
