...
Peach_64: {
	...
	config: [
		...
		// Peach_64 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Peach_64 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/peach-64/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/peach-64$"
					to:            route_match.path
		...
		// greymatter catalog service definition for peach-64
		#catalog_entry & {
			name:                      Name
		...
		// Shared peach-64 proxy object. All configs
