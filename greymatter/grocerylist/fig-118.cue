...
Fig_118: {
	...
	config: [
		...
		// Fig_118 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_118 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-118/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-118$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-118
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-118 proxy object. All configs
