...
Grape_132: {
	...
	config: [
		...
		// Grape_132 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Grape_132 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/grape-132/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/grape-132$"
					to:            route_match.path
		...
		// greymatter catalog service definition for grape-132
		#catalog_entry & {
			name:                      Name
		...
		// Shared grape-132 proxy object. All configs
