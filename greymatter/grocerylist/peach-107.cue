...
Peach_107: {
	...
	config: [
		...
		// Peach_107 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Peach_107 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/peach-107/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/peach-107$"
					to:            route_match.path
		...
		// greymatter catalog service definition for peach-107
		#catalog_entry & {
			name:                      Name
		...
		// Shared peach-107 proxy object. All configs
