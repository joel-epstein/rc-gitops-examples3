...
Grape_135: {
	...
	config: [
		...
		// Grape_135 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Grape_135 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/grape-135/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/grape-135$"
					to:            route_match.path
		...
		// greymatter catalog service definition for grape-135
		#catalog_entry & {
			name:                      Name
		...
		// Shared grape-135 proxy object. All configs
