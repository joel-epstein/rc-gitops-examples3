...
Star_147: {
	...
	config: [
		...
		// Star_147 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_147 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-147/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-147$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-147
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-147 proxy object. All configs
