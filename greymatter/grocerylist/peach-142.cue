...
Peach_142: {
	...
	config: [
		...
		// Peach_142 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Peach_142 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/peach-142/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/peach-142$"
					to:            route_match.path
		...
		// greymatter catalog service definition for peach-142
		#catalog_entry & {
			name:                      Name
		...
		// Shared peach-142 proxy object. All configs
