...
Peach_122: {
	...
	config: [
		...
		// Peach_122 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Peach_122 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/peach-122/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/peach-122$"
					to:            route_match.path
		...
		// greymatter catalog service definition for peach-122
		#catalog_entry & {
			name:                      Name
		...
		// Shared peach-122 proxy object. All configs
