...
Fig_121: {
	...
	config: [
		...
		// Fig_121 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_121 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-121/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-121$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-121
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-121 proxy object. All configs
