...
Peach_61: {
	...
	config: [
		...
		// Peach_61 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Peach_61 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/peach-61/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/peach-61$"
					to:            route_match.path
		...
		// greymatter catalog service definition for peach-61
		#catalog_entry & {
			name:                      Name
		...
		// Shared peach-61 proxy object. All configs
