...
Star_103: {
	...
	config: [
		...
		// Star_103 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_103 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-103/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-103$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-103
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-103 proxy object. All configs
