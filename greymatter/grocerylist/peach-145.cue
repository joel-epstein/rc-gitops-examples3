...
Peach_145: {
	...
	config: [
		...
		// Peach_145 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Peach_145 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/peach-145/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/peach-145$"
					to:            route_match.path
		...
		// greymatter catalog service definition for peach-145
		#catalog_entry & {
			name:                      Name
		...
		// Shared peach-145 proxy object. All configs
