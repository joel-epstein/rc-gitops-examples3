...
Grape_101: {
	...
	config: [
		...
		// Grape_101 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Grape_101 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/grape-101/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/grape-101$"
					to:            route_match.path
		...
		// greymatter catalog service definition for grape-101
		#catalog_entry & {
			name:                      Name
		...
		// Shared grape-101 proxy object. All configs
