...
Grape_108: {
	...
	config: [
		...
		// Grape_108 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Grape_108 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/grape-108/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/grape-108$"
					to:            route_match.path
		...
		// greymatter catalog service definition for grape-108
		#catalog_entry & {
			name:                      Name
		...
		// Shared grape-108 proxy object. All configs
