...
Grape_104: {
	...
	config: [
		...
		// Grape_104 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Grape_104 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/grape-104/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/grape-104$"
					to:            route_match.path
		...
		// greymatter catalog service definition for grape-104
		#catalog_entry & {
			name:                      Name
		...
		// Shared grape-104 proxy object. All configs
