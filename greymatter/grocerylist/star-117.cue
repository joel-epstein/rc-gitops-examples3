...
Star_117: {
	...
	config: [
		...
		// Star_117 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_117 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-117/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-117$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-117
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-117 proxy object. All configs
