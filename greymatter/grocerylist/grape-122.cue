...
Grape_122: {
	...
	config: [
		...
		// Grape_122 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Grape_122 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/grape-122/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/grape-122$"
					to:            route_match.path
		...
		// greymatter catalog service definition for grape-122
		#catalog_entry & {
			name:                      Name
		...
		// Shared grape-122 proxy object. All configs
