...
Fig_138: {
	...
	config: [
		...
		// Fig_138 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_138 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-138/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-138$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-138
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-138 proxy object. All configs
