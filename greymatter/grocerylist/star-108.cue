...
Star_108: {
	...
	config: [
		...
		// Star_108 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_108 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-108/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-108$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-108
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-108 proxy object. All configs
