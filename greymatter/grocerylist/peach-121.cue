...
Peach_121: {
	...
	config: [
		...
		// Peach_121 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Peach_121 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/peach-121/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/peach-121$"
					to:            route_match.path
		...
		// greymatter catalog service definition for peach-121
		#catalog_entry & {
			name:                      Name
		...
		// Shared peach-121 proxy object. All configs
