...
Peach_79: {
	...
	config: [
		...
		// Peach_79 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Peach_79 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/peach-79/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/peach-79$"
					to:            route_match.path
		...
		// greymatter catalog service definition for peach-79
		#catalog_entry & {
			name:                      Name
		...
		// Shared peach-79 proxy object. All configs
