...
Fig_147: {
	...
	config: [
		...
		// Fig_147 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_147 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-147/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-147$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-147
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-147 proxy object. All configs
