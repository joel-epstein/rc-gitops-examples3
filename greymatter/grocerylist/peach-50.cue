...
Peach_50: {
	...
	config: [
		...
		// Peach_50 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Peach_50 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/peach-50/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/peach-50$"
					to:            route_match.path
		...
		// greymatter catalog service definition for peach-50
		#catalog_entry & {
			name:                      Name
		...
		// Shared peach-50 proxy object. All configs
