...
Star_141: {
	...
	config: [
		...
		// Star_141 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_141 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-141/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-141$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-141
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-141 proxy object. All configs
