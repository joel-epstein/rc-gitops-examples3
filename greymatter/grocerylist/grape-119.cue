...
Grape_119: {
	...
	config: [
		...
		// Grape_119 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Grape_119 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/grape-119/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/grape-119$"
					to:            route_match.path
		...
		// greymatter catalog service definition for grape-119
		#catalog_entry & {
			name:                      Name
		...
		// Shared grape-119 proxy object. All configs
