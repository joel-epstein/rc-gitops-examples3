...
Grape_144: {
	...
	config: [
		...
		// Grape_144 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Grape_144 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/grape-144/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/grape-144$"
					to:            route_match.path
		...
		// greymatter catalog service definition for grape-144
		#catalog_entry & {
			name:                      Name
		...
		// Shared grape-144 proxy object. All configs
