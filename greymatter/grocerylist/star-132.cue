...
Star_132: {
	...
	config: [
		...
		// Star_132 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_132 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-132/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-132$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-132
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-132 proxy object. All configs
