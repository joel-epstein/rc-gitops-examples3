...
Peach_68: {
	...
	config: [
		...
		// Peach_68 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Peach_68 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/peach-68/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/peach-68$"
					to:            route_match.path
		...
		// greymatter catalog service definition for peach-68
		#catalog_entry & {
			name:                      Name
		...
		// Shared peach-68 proxy object. All configs
