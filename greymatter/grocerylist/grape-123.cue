...
Grape_123: {
	...
	config: [
		...
		// Grape_123 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Grape_123 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/grape-123/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/grape-123$"
					to:            route_match.path
		...
		// greymatter catalog service definition for grape-123
		#catalog_entry & {
			name:                      Name
		...
		// Shared grape-123 proxy object. All configs
