...
Peach_20: {
	...
	config: [
		...
		// Peach_20 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Peach_20 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/peach-20/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/peach-20$"
					to:            route_match.path
		...
		// greymatter catalog service definition for peach-20
		#catalog_entry & {
			name:                      Name
		...
		// Shared peach-20 proxy object. All configs
