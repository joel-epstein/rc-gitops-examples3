...
Star_110: {
	...
	config: [
		...
		// Star_110 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_110 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-110/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-110$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-110
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-110 proxy object. All configs
