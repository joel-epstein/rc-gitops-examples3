...
Peach_109: {
	...
	config: [
		...
		// Peach_109 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Peach_109 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/peach-109/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/peach-109$"
					to:            route_match.path
		...
		// greymatter catalog service definition for peach-109
		#catalog_entry & {
			name:                      Name
		...
		// Shared peach-109 proxy object. All configs
