...
Fig_141: {
	...
	config: [
		...
		// Fig_141 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_141 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-141/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-141$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-141
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-141 proxy object. All configs
