...
Star_121: {
	...
	config: [
		...
		// Star_121 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_121 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-121/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-121$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-121
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-121 proxy object. All configs
