...
Peach_138: {
	...
	config: [
		...
		// Peach_138 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Peach_138 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/peach-138/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/peach-138$"
					to:            route_match.path
		...
		// greymatter catalog service definition for peach-138
		#catalog_entry & {
			name:                      Name
		...
		// Shared peach-138 proxy object. All configs
