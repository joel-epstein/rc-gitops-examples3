...
Fig_123: {
	...
	config: [
		...
		// Fig_123 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_123 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-123/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-123$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-123
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-123 proxy object. All configs
