...
Fig_125: {
	...
	config: [
		...
		// Fig_125 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_125 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-125/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-125$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-125
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-125 proxy object. All configs
