...
Peach_2: {
	...
	config: [
		...
		// Peach_2 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Peach_2 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/peach-2/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/peach-2$"
					to:            route_match.path
		...
		// greymatter catalog service definition for peach-2
		#catalog_entry & {
			name:                      Name
		...
		// Shared peach-2 proxy object. All configs
