...
Star_101: {
	...
	config: [
		...
		// Star_101 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_101 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-101/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-101$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-101
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-101 proxy object. All configs
