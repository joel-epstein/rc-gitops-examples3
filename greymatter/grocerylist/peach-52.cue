...
Peach_52: {
	...
	config: [
		...
		// Peach_52 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Peach_52 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/peach-52/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/peach-52$"
					to:            route_match.path
		...
		// greymatter catalog service definition for peach-52
		#catalog_entry & {
			name:                      Name
		...
		// Shared peach-52 proxy object. All configs
