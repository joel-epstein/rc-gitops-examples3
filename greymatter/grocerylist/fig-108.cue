...
Fig_108: {
	...
	config: [
		...
		// Fig_108 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_108 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-108/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-108$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-108
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-108 proxy object. All configs
