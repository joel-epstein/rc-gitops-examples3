...
Peach_108: {
	...
	config: [
		...
		// Peach_108 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Peach_108 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/peach-108/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/peach-108$"
					to:            route_match.path
		...
		// greymatter catalog service definition for peach-108
		#catalog_entry & {
			name:                      Name
		...
		// Shared peach-108 proxy object. All configs
