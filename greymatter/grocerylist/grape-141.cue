...
Grape_141: {
	...
	config: [
		...
		// Grape_141 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Grape_141 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/grape-141/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/grape-141$"
					to:            route_match.path
		...
		// greymatter catalog service definition for grape-141
		#catalog_entry & {
			name:                      Name
		...
		// Shared grape-141 proxy object. All configs
