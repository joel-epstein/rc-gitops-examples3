...
Peach_99: {
	...
	config: [
		...
		// Peach_99 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Peach_99 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/peach-99/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/peach-99$"
					to:            route_match.path
		...
		// greymatter catalog service definition for peach-99
		#catalog_entry & {
			name:                      Name
		...
		// Shared peach-99 proxy object. All configs
