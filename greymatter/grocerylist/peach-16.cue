...
Peach_16: {
	...
	config: [
		...
		// Peach_16 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Peach_16 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/peach-16/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/peach-16$"
					to:            route_match.path
		...
		// greymatter catalog service definition for peach-16
		#catalog_entry & {
			name:                      Name
		...
		// Shared peach-16 proxy object. All configs
