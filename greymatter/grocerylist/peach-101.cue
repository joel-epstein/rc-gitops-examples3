...
Peach_101: {
	...
	config: [
		...
		// Peach_101 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Peach_101 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/peach-101/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/peach-101$"
					to:            route_match.path
		...
		// greymatter catalog service definition for peach-101
		#catalog_entry & {
			name:                      Name
		...
		// Shared peach-101 proxy object. All configs
