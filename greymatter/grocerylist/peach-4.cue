...
Peach_4: {
	...
	config: [
		...
		// Peach_4 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Peach_4 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/peach-4/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/peach-4$"
					to:            route_match.path
		...
		// greymatter catalog service definition for peach-4
		#catalog_entry & {
			name:                      Name
		...
		// Shared peach-4 proxy object. All configs
