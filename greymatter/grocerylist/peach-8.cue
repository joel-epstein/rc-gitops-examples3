...
Peach_8: {
	...
	config: [
		...
		// Peach_8 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Peach_8 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/peach-8/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/peach-8$"
					to:            route_match.path
		...
		// greymatter catalog service definition for peach-8
		#catalog_entry & {
			name:                      Name
		...
		// Shared peach-8 proxy object. All configs
