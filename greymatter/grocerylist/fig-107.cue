...
Fig_107: {
	...
	config: [
		...
		// Fig_107 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_107 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-107/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-107$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-107
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-107 proxy object. All configs
