...
Star_133: {
	...
	config: [
		...
		// Star_133 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_133 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-133/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-133$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-133
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-133 proxy object. All configs
