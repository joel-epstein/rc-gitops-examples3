...
Peach_133: {
	...
	config: [
		...
		// Peach_133 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Peach_133 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/peach-133/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/peach-133$"
					to:            route_match.path
		...
		// greymatter catalog service definition for peach-133
		#catalog_entry & {
			name:                      Name
		...
		// Shared peach-133 proxy object. All configs
