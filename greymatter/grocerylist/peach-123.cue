...
Peach_123: {
	...
	config: [
		...
		// Peach_123 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Peach_123 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/peach-123/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/peach-123$"
					to:            route_match.path
		...
		// greymatter catalog service definition for peach-123
		#catalog_entry & {
			name:                      Name
		...
		// Shared peach-123 proxy object. All configs
