...
Peach_30: {
	...
	config: [
		...
		// Peach_30 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Peach_30 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/peach-30/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/peach-30$"
					to:            route_match.path
		...
		// greymatter catalog service definition for peach-30
		#catalog_entry & {
			name:                      Name
		...
		// Shared peach-30 proxy object. All configs
