...
Peach_141: {
	...
	config: [
		...
		// Peach_141 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Peach_141 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/peach-141/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/peach-141$"
					to:            route_match.path
		...
		// greymatter catalog service definition for peach-141
		#catalog_entry & {
			name:                      Name
		...
		// Shared peach-141 proxy object. All configs
