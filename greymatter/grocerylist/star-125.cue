...
Star_125: {
	...
	config: [
		...
		// Star_125 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_125 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-125/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-125$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-125
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-125 proxy object. All configs
