...
Peach_55: {
	...
	config: [
		...
		// Peach_55 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Peach_55 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/peach-55/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/peach-55$"
					to:            route_match.path
		...
		// greymatter catalog service definition for peach-55
		#catalog_entry & {
			name:                      Name
		...
		// Shared peach-55 proxy object. All configs
