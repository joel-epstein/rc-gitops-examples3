...
Star_134: {
	...
	config: [
		...
		// Star_134 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_134 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-134/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-134$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-134
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-134 proxy object. All configs
