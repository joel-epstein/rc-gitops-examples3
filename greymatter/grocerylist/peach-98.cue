...
Peach_98: {
	...
	config: [
		...
		// Peach_98 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Peach_98 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/peach-98/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/peach-98$"
					to:            route_match.path
		...
		// greymatter catalog service definition for peach-98
		#catalog_entry & {
			name:                      Name
		...
		// Shared peach-98 proxy object. All configs
