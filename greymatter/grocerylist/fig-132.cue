...
Fig_132: {
	...
	config: [
		...
		// Fig_132 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_132 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-132/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-132$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-132
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-132 proxy object. All configs
