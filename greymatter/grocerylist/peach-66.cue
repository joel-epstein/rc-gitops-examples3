...
Peach_66: {
	...
	config: [
		...
		// Peach_66 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Peach_66 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/peach-66/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/peach-66$"
					to:            route_match.path
		...
		// greymatter catalog service definition for peach-66
		#catalog_entry & {
			name:                      Name
		...
		// Shared peach-66 proxy object. All configs
