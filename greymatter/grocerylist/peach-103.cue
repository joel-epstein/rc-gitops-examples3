...
Peach_103: {
	...
	config: [
		...
		// Peach_103 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Peach_103 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/peach-103/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/peach-103$"
					to:            route_match.path
		...
		// greymatter catalog service definition for peach-103
		#catalog_entry & {
			name:                      Name
		...
		// Shared peach-103 proxy object. All configs
