...
Peach_127: {
	...
	config: [
		...
		// Peach_127 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Peach_127 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/peach-127/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/peach-127$"
					to:            route_match.path
		...
		// greymatter catalog service definition for peach-127
		#catalog_entry & {
			name:                      Name
		...
		// Shared peach-127 proxy object. All configs
