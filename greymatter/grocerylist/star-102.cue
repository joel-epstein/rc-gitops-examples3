...
Star_102: {
	...
	config: [
		...
		// Star_102 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_102 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-102/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-102$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-102
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-102 proxy object. All configs
