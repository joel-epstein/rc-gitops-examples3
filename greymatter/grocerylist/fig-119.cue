...
Fig_119: {
	...
	config: [
		...
		// Fig_119 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_119 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-119/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-119$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-119
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-119 proxy object. All configs
