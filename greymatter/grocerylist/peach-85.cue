...
Peach_85: {
	...
	config: [
		...
		// Peach_85 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Peach_85 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/peach-85/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/peach-85$"
					to:            route_match.path
		...
		// greymatter catalog service definition for peach-85
		#catalog_entry & {
			name:                      Name
		...
		// Shared peach-85 proxy object. All configs
