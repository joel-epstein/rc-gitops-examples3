...
Peach_6: {
	...
	config: [
		...
		// Peach_6 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Peach_6 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/peach-6/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/peach-6$"
					to:            route_match.path
		...
		// greymatter catalog service definition for peach-6
		#catalog_entry & {
			name:                      Name
		...
		// Shared peach-6 proxy object. All configs
