...
Peach_134: {
	...
	config: [
		...
		// Peach_134 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Peach_134 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/peach-134/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/peach-134$"
					to:            route_match.path
		...
		// greymatter catalog service definition for peach-134
		#catalog_entry & {
			name:                      Name
		...
		// Shared peach-134 proxy object. All configs
