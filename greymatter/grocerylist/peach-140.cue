...
Peach_140: {
	...
	config: [
		...
		// Peach_140 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Peach_140 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/peach-140/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/peach-140$"
					to:            route_match.path
		...
		// greymatter catalog service definition for peach-140
		#catalog_entry & {
			name:                      Name
		...
		// Shared peach-140 proxy object. All configs
