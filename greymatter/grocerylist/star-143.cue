...
Star_143: {
	...
	config: [
		...
		// Star_143 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_143 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-143/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-143$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-143
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-143 proxy object. All configs
