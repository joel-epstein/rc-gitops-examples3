...
Star_129: {
	...
	config: [
		...
		// Star_129 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_129 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-129/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-129$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-129
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-129 proxy object. All configs
