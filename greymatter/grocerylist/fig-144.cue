...
Fig_144: {
	...
	config: [
		...
		// Fig_144 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_144 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-144/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-144$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-144
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-144 proxy object. All configs
