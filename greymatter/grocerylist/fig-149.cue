...
Fig_149: {
	...
	config: [
		...
		// Fig_149 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_149 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-149/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-149$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-149
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-149 proxy object. All configs
