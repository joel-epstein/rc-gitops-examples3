...
Fig_120: {
	...
	config: [
		...
		// Fig_120 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_120 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-120/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-120$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-120
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-120 proxy object. All configs
