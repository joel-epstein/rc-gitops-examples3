...
Peach_89: {
	...
	config: [
		...
		// Peach_89 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Peach_89 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/peach-89/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/peach-89$"
					to:            route_match.path
		...
		// greymatter catalog service definition for peach-89
		#catalog_entry & {
			name:                      Name
		...
		// Shared peach-89 proxy object. All configs
