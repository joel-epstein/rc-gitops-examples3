...
Fig_129: {
	...
	config: [
		...
		// Fig_129 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_129 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-129/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-129$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-129
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-129 proxy object. All configs
