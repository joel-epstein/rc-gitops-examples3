...
Grape_134: {
	...
	config: [
		...
		// Grape_134 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Grape_134 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/grape-134/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/grape-134$"
					to:            route_match.path
		...
		// greymatter catalog service definition for grape-134
		#catalog_entry & {
			name:                      Name
		...
		// Shared grape-134 proxy object. All configs
