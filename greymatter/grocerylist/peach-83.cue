...
Peach_83: {
	...
	config: [
		...
		// Peach_83 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Peach_83 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/peach-83/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/peach-83$"
					to:            route_match.path
		...
		// greymatter catalog service definition for peach-83
		#catalog_entry & {
			name:                      Name
		...
		// Shared peach-83 proxy object. All configs
