...
Star_140: {
	...
	config: [
		...
		// Star_140 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_140 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-140/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-140$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-140
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-140 proxy object. All configs
