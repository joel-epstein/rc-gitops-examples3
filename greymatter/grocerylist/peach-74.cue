...
Peach_74: {
	...
	config: [
		...
		// Peach_74 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Peach_74 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/peach-74/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/peach-74$"
					to:            route_match.path
		...
		// greymatter catalog service definition for peach-74
		#catalog_entry & {
			name:                      Name
		...
		// Shared peach-74 proxy object. All configs
