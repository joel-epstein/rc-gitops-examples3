...
Peach_88: {
	...
	config: [
		...
		// Peach_88 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Peach_88 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/peach-88/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/peach-88$"
					to:            route_match.path
		...
		// greymatter catalog service definition for peach-88
		#catalog_entry & {
			name:                      Name
		...
		// Shared peach-88 proxy object. All configs
