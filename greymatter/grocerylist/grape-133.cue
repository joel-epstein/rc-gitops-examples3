...
Grape_133: {
	...
	config: [
		...
		// Grape_133 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Grape_133 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/grape-133/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/grape-133$"
					to:            route_match.path
		...
		// greymatter catalog service definition for grape-133
		#catalog_entry & {
			name:                      Name
		...
		// Shared grape-133 proxy object. All configs
