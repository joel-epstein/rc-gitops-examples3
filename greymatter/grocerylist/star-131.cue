...
Star_131: {
	...
	config: [
		...
		// Star_131 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_131 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-131/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-131$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-131
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-131 proxy object. All configs
