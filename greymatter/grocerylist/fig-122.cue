...
Fig_122: {
	...
	config: [
		...
		// Fig_122 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_122 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-122/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-122$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-122
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-122 proxy object. All configs
