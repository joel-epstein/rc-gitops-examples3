...
Grape_106: {
	...
	config: [
		...
		// Grape_106 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Grape_106 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/grape-106/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/grape-106$"
					to:            route_match.path
		...
		// greymatter catalog service definition for grape-106
		#catalog_entry & {
			name:                      Name
		...
		// Shared grape-106 proxy object. All configs
