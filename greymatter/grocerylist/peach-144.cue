...
Peach_144: {
	...
	config: [
		...
		// Peach_144 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Peach_144 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/peach-144/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/peach-144$"
					to:            route_match.path
		...
		// greymatter catalog service definition for peach-144
		#catalog_entry & {
			name:                      Name
		...
		// Shared peach-144 proxy object. All configs
