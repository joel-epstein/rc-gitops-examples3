...
Fig_142: {
	...
	config: [
		...
		// Fig_142 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_142 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-142/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-142$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-142
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-142 proxy object. All configs
