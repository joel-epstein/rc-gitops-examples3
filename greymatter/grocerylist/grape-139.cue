...
Grape_139: {
	...
	config: [
		...
		// Grape_139 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Grape_139 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/grape-139/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/grape-139$"
					to:            route_match.path
		...
		// greymatter catalog service definition for grape-139
		#catalog_entry & {
			name:                      Name
		...
		// Shared grape-139 proxy object. All configs
