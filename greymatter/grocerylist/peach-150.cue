...
Peach_150: {
	...
	config: [
		...
		// Peach_150 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Peach_150 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/peach-150/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/peach-150$"
					to:            route_match.path
		...
		// greymatter catalog service definition for peach-150
		#catalog_entry & {
			name:                      Name
		...
		// Shared peach-150 proxy object. All configs
