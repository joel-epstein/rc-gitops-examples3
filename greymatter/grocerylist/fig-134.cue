...
Fig_134: {
	...
	config: [
		...
		// Fig_134 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_134 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-134/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-134$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-134
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-134 proxy object. All configs
