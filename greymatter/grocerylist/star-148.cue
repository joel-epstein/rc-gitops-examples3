...
Star_148: {
	...
	config: [
		...
		// Star_148 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_148 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-148/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-148$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-148
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-148 proxy object. All configs
