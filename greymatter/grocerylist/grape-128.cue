...
Grape_128: {
	...
	config: [
		...
		// Grape_128 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Grape_128 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/grape-128/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/grape-128$"
					to:            route_match.path
		...
		// greymatter catalog service definition for grape-128
		#catalog_entry & {
			name:                      Name
		...
		// Shared grape-128 proxy object. All configs
