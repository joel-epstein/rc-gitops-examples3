...
Peach_90: {
	...
	config: [
		...
		// Peach_90 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Peach_90 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/peach-90/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/peach-90$"
					to:            route_match.path
		...
		// greymatter catalog service definition for peach-90
		#catalog_entry & {
			name:                      Name
		...
		// Shared peach-90 proxy object. All configs
