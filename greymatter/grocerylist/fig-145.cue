...
Fig_145: {
	...
	config: [
		...
		// Fig_145 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_145 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-145/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-145$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-145
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-145 proxy object. All configs
