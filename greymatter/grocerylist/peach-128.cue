...
Peach_128: {
	...
	config: [
		...
		// Peach_128 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Peach_128 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/peach-128/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/peach-128$"
					to:            route_match.path
		...
		// greymatter catalog service definition for peach-128
		#catalog_entry & {
			name:                      Name
		...
		// Shared peach-128 proxy object. All configs
