...
Peach_120: {
	...
	config: [
		...
		// Peach_120 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Peach_120 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/peach-120/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/peach-120$"
					to:            route_match.path
		...
		// greymatter catalog service definition for peach-120
		#catalog_entry & {
			name:                      Name
		...
		// Shared peach-120 proxy object. All configs
