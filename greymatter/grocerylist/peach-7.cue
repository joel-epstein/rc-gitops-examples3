...
Peach_7: {
	...
	config: [
		...
		// Peach_7 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Peach_7 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/peach-7/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/peach-7$"
					to:            route_match.path
		...
		// greymatter catalog service definition for peach-7
		#catalog_entry & {
			name:                      Name
		...
		// Shared peach-7 proxy object. All configs
