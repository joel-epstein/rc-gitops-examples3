...
Grape_121: {
	...
	config: [
		...
		// Grape_121 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Grape_121 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/grape-121/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/grape-121$"
					to:            route_match.path
		...
		// greymatter catalog service definition for grape-121
		#catalog_entry & {
			name:                      Name
		...
		// Shared grape-121 proxy object. All configs
