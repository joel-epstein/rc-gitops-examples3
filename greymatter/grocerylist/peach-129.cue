...
Peach_129: {
	...
	config: [
		...
		// Peach_129 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Peach_129 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/peach-129/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/peach-129$"
					to:            route_match.path
		...
		// greymatter catalog service definition for peach-129
		#catalog_entry & {
			name:                      Name
		...
		// Shared peach-129 proxy object. All configs
