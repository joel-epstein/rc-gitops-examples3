...
Peach_125: {
	...
	config: [
		...
		// Peach_125 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Peach_125 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/peach-125/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/peach-125$"
					to:            route_match.path
		...
		// greymatter catalog service definition for peach-125
		#catalog_entry & {
			name:                      Name
		...
		// Shared peach-125 proxy object. All configs
