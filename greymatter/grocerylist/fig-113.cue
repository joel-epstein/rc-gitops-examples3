...
Fig_113: {
	...
	config: [
		...
		// Fig_113 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_113 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-113/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-113$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-113
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-113 proxy object. All configs
