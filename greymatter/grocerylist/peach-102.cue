...
Peach_102: {
	...
	config: [
		...
		// Peach_102 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Peach_102 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/peach-102/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/peach-102$"
					to:            route_match.path
		...
		// greymatter catalog service definition for peach-102
		#catalog_entry & {
			name:                      Name
		...
		// Shared peach-102 proxy object. All configs
