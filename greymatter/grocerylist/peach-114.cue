...
Peach_114: {
	...
	config: [
		...
		// Peach_114 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Peach_114 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/peach-114/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/peach-114$"
					to:            route_match.path
		...
		// greymatter catalog service definition for peach-114
		#catalog_entry & {
			name:                      Name
		...
		// Shared peach-114 proxy object. All configs
