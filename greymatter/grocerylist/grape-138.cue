...
Grape_138: {
	...
	config: [
		...
		// Grape_138 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Grape_138 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/grape-138/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/grape-138$"
					to:            route_match.path
		...
		// greymatter catalog service definition for grape-138
		#catalog_entry & {
			name:                      Name
		...
		// Shared grape-138 proxy object. All configs
