...
Peach_56: {
	...
	config: [
		...
		// Peach_56 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Peach_56 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/peach-56/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/peach-56$"
					to:            route_match.path
		...
		// greymatter catalog service definition for peach-56
		#catalog_entry & {
			name:                      Name
		...
		// Shared peach-56 proxy object. All configs
