...
Peach_78: {
	...
	config: [
		...
		// Peach_78 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Peach_78 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/peach-78/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/peach-78$"
					to:            route_match.path
		...
		// greymatter catalog service definition for peach-78
		#catalog_entry & {
			name:                      Name
		...
		// Shared peach-78 proxy object. All configs
