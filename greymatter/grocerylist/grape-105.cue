...
Grape_105: {
	...
	config: [
		...
		// Grape_105 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Grape_105 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/grape-105/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/grape-105$"
					to:            route_match.path
		...
		// greymatter catalog service definition for grape-105
		#catalog_entry & {
			name:                      Name
		...
		// Shared grape-105 proxy object. All configs
