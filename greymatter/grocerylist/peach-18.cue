...
Peach_18: {
	...
	config: [
		...
		// Peach_18 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Peach_18 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/peach-18/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/peach-18$"
					to:            route_match.path
		...
		// greymatter catalog service definition for peach-18
		#catalog_entry & {
			name:                      Name
		...
		// Shared peach-18 proxy object. All configs
