...
Peach_116: {
	...
	config: [
		...
		// Peach_116 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Peach_116 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/peach-116/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/peach-116$"
					to:            route_match.path
		...
		// greymatter catalog service definition for peach-116
		#catalog_entry & {
			name:                      Name
		...
		// Shared peach-116 proxy object. All configs
