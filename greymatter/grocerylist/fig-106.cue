...
Fig_106: {
	...
	config: [
		...
		// Fig_106 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_106 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-106/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-106$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-106
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-106 proxy object. All configs
