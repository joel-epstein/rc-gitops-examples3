...
Fig_126: {
	...
	config: [
		...
		// Fig_126 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_126 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-126/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-126$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-126
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-126 proxy object. All configs
