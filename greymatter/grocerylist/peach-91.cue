...
Peach_91: {
	...
	config: [
		...
		// Peach_91 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Peach_91 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/peach-91/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/peach-91$"
					to:            route_match.path
		...
		// greymatter catalog service definition for peach-91
		#catalog_entry & {
			name:                      Name
		...
		// Shared peach-91 proxy object. All configs
