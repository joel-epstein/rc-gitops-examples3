...
Peach_26: {
	...
	config: [
		...
		// Peach_26 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Peach_26 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/peach-26/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/peach-26$"
					to:            route_match.path
		...
		// greymatter catalog service definition for peach-26
		#catalog_entry & {
			name:                      Name
		...
		// Shared peach-26 proxy object. All configs
