...
Peach_113: {
	...
	config: [
		...
		// Peach_113 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Peach_113 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/peach-113/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/peach-113$"
					to:            route_match.path
		...
		// greymatter catalog service definition for peach-113
		#catalog_entry & {
			name:                      Name
		...
		// Shared peach-113 proxy object. All configs
