...
Peach_54: {
	...
	config: [
		...
		// Peach_54 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Peach_54 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/peach-54/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/peach-54$"
					to:            route_match.path
		...
		// greymatter catalog service definition for peach-54
		#catalog_entry & {
			name:                      Name
		...
		// Shared peach-54 proxy object. All configs
