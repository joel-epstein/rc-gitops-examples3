...
Grape_125: {
	...
	config: [
		...
		// Grape_125 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Grape_125 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/grape-125/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/grape-125$"
					to:            route_match.path
		...
		// greymatter catalog service definition for grape-125
		#catalog_entry & {
			name:                      Name
		...
		// Shared grape-125 proxy object. All configs
