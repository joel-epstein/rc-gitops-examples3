...
Peach_60: {
	...
	config: [
		...
		// Peach_60 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Peach_60 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/peach-60/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/peach-60$"
					to:            route_match.path
		...
		// greymatter catalog service definition for peach-60
		#catalog_entry & {
			name:                      Name
		...
		// Shared peach-60 proxy object. All configs
