...
Grape_127: {
	...
	config: [
		...
		// Grape_127 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Grape_127 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/grape-127/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/grape-127$"
					to:            route_match.path
		...
		// greymatter catalog service definition for grape-127
		#catalog_entry & {
			name:                      Name
		...
		// Shared grape-127 proxy object. All configs
