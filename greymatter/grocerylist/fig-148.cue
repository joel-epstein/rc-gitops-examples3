...
Fig_148: {
	...
	config: [
		...
		// Fig_148 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_148 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-148/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-148$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-148
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-148 proxy object. All configs
