...
Peach_70: {
	...
	config: [
		...
		// Peach_70 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Peach_70 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/peach-70/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/peach-70$"
					to:            route_match.path
		...
		// greymatter catalog service definition for peach-70
		#catalog_entry & {
			name:                      Name
		...
		// Shared peach-70 proxy object. All configs
