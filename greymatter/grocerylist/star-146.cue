...
Star_146: {
	...
	config: [
		...
		// Star_146 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_146 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-146/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-146$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-146
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-146 proxy object. All configs
