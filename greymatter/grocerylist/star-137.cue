...
Star_137: {
	...
	config: [
		...
		// Star_137 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_137 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-137/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-137$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-137
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-137 proxy object. All configs
