...
Star_115: {
	...
	config: [
		...
		// Star_115 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_115 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-115/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-115$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-115
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-115 proxy object. All configs
