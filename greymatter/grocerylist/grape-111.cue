...
Grape_111: {
	...
	config: [
		...
		// Grape_111 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Grape_111 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/grape-111/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/grape-111$"
					to:            route_match.path
		...
		// greymatter catalog service definition for grape-111
		#catalog_entry & {
			name:                      Name
		...
		// Shared grape-111 proxy object. All configs
