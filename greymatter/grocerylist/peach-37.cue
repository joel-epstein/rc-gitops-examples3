...
Peach_37: {
	...
	config: [
		...
		// Peach_37 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Peach_37 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/peach-37/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/peach-37$"
					to:            route_match.path
		...
		// greymatter catalog service definition for peach-37
		#catalog_entry & {
			name:                      Name
		...
		// Shared peach-37 proxy object. All configs
