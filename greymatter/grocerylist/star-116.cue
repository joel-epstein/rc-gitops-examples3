...
Star_116: {
	...
	config: [
		...
		// Star_116 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_116 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-116/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-116$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-116
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-116 proxy object. All configs
