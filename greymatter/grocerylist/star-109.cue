...
Star_109: {
	...
	config: [
		...
		// Star_109 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Star_109 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/star-109/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/star-109$"
					to:            route_match.path
		...
		// greymatter catalog service definition for star-109
		#catalog_entry & {
			name:                      Name
		...
		// Shared star-109 proxy object. All configs
