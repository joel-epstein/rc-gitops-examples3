...
Fig_136: {
	...
	config: [
		...
		// Fig_136 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_136 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-136/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-136$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-136
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-136 proxy object. All configs
