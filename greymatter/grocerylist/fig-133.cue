...
Fig_133: {
	...
	config: [
		...
		// Fig_133 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_133 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-133/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-133$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-133
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-133 proxy object. All configs
