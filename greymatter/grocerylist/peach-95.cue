...
Peach_95: {
	...
	config: [
		...
		// Peach_95 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Peach_95 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/peach-95/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/peach-95$"
					to:            route_match.path
		...
		// greymatter catalog service definition for peach-95
		#catalog_entry & {
			name:                      Name
		...
		// Shared peach-95 proxy object. All configs
