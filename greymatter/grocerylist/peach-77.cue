...
Peach_77: {
	...
	config: [
		...
		// Peach_77 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Peach_77 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/peach-77/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/peach-77$"
					to:            route_match.path
		...
		// greymatter catalog service definition for peach-77
		#catalog_entry & {
			name:                      Name
		...
		// Shared peach-77 proxy object. All configs
