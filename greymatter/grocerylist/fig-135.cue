...
Fig_135: {
	...
	config: [
		...
		// Fig_135 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_135 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-135/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-135$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-135
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-135 proxy object. All configs
