...
Peach_1: {
	...
	config: [
		...
		// Peach_1 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Peach_1 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/peach-1/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/peach-1$"
					to:            route_match.path
		...
		// greymatter catalog service definition for peach-1
		#catalog_entry & {
			name:                      Name
		...
		// Shared peach-1 proxy object. All configs
