...
Fig_103: {
	...
	config: [
		...
		// Fig_103 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Fig_103 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/fig-103/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/fig-103$"
					to:            route_match.path
		...
		// greymatter catalog service definition for fig-103
		#catalog_entry & {
			name:                      Name
		...
		// Shared fig-103 proxy object. All configs
