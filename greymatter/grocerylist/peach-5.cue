...
Peach_5: {
	...
	config: [
		...
		// Peach_5 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Peach_5 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/peach-5/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/peach-5$"
					to:            route_match.path
		...
		// greymatter catalog service definition for peach-5
		#catalog_entry & {
			name:                      Name
		...
		// Shared peach-5 proxy object. All configs
