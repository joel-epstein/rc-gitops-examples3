...
Peach_15: {
	...
	config: [
		...
		// Peach_15 -> HTTP ingress to your container
		#domain & {
		...
		// Edge config for the Peach_15 service.
		...
		#route & {
			...
			route_key:  Name
			route_match: {
				path: "/services/grocerylist/peach-15/"
			}
			redirects: [
				{
					from:          "^/services/grocerylist/peach-15$"
					to:            route_match.path
		...
		// greymatter catalog service definition for peach-15
		#catalog_entry & {
			name:                      Name
		...
		// Shared peach-15 proxy object. All configs
